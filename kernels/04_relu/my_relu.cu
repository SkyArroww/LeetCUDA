...
__global__ void relu_f16_kernel(half *x, half *y, int N){
    const half h0 = __float2half(0.0f);
    int idx = blockDim.x * blockIdx.x + threadIdx.x;
    if(idx < N){
        y[idx] = __hmax(h0, x[idx]);
    }
}
...
__global__ void relu_f16x2_kernel(half *x, half *y, int N){
    ...
    if(idx + 1 < N){
        ...
        reg_y.x = __hmax(h0, reg_x.x);
        reg_y.y = __hmax(h0, reg_x.y);
        HALF2(y[idx]) = reg_y;
    }
}
...
__global__ void relu_f16x8_pack_kernel(half *x, half *y, int N){
    const half2 h0_2 = {__float2half(0.0f), __float2half(0.0f)};
    ...
    if(idx + 7 < N){
        ...
        #pragma unroll
        for(int i = 0; i < 8; i += 2){
            HALF2(pack_y[i]) = __hmax2(HALF2(pack_x[i]), h0_2);
        }
