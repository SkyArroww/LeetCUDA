...
// Block reduce sum device helper for Layer Norm
...
__device__ float block_reduce_sum_f32(float val){
    ...
    static __shared__ float shared_smem[NUM_WARPS];

    val = warp_reduce_sum_f32<WARP_SIZE>(val);
    if(lane == 0)
        shared_smem[warp] = val;
    __syncthreads();
    val = (lane < NUM_WARPS) ? shared_smem[lane] : 0.f;
...
__global__ void layer_norm_f32_kernel(float *x, float *y, float g, float b, int N, int K){
    ...
    float val = (idx < N * K) ? x[idx] : 0.f;
    float sum = block_reduce_sum_f32<NUM_THREADS>(val);
    if(tid == 0)
        shared_mean = sum / (float)K;
    __syncthreads();
    float var = (val - shared_mean) * (val - shared_mean);
    var = block_reduce_sum_f32<NUM_THREADS>(var);

    const float epsilon = 1e-5f;
    if(tid == 0)
        shared_var = rsqrtf(var / (float)K + epsilon);
    __syncthreads();
    if(idx < N * K)
        y[idx] = ((val - shared_mean) * shared_var) * g + b;
}
...
__global__ void layer_norm_f32x4_kernel(float *x, float *y, float g, float b, int N, int K){
    int tid = threadIdx.x;
    int bid = blockIdx.x;
    int idx = 4 * (blockDim.x * bid + tid);
    const float epsilon = 1e-5f;

    __shared__ float shared_mean;
    __shared__ float shared_var;
    float4 reg_x = FLOAT4(x[idx]);
    float value = (idx + 3 <  N * K) ? (reg_x.x + reg_x.y + reg_x.z + reg_x.w) : 0.f;
    float sum = block_reduce_sum_f32<NUM_THREADS>(value);
    if(tid == 0)
        shared_mean = sum / (float)K;
    __syncthreads();
    float4 reg_x_hat;
    reg_x_hat.x = reg_x.x - shared_mean;
    reg_x_hat.y = reg_x.y - shared_mean;
    reg_x_hat.z = reg_x.z - shared_mean;
    reg_x_hat.w = reg_x.w - shared_mean;
    float var = reg_x_hat.x * reg_x_hat.x + reg_x_hat.y * reg_x_hat.y +
    ...
    __syncthreads();
    float4 reg_y;
    reg_y.x = reg_x_hat.x * shared_var * g + b;
    reg_y.y = reg_x_hat.y * shared_var * g + b;
    reg_y.z = reg_x_hat.z * shared_var * g + b;
    reg_y.w = reg_x_hat.w * shared_var * g + b;
    if(idx < N * K)
        FLOAT4(y[idx]) = reg_y;
}
...
__device__ half block_reduce_sum_f16_f16(half val){
    ...
    if(lane == 0)
        shared[warp] = val;
    __syncthreads();
    val = (lane < NUM_WARPS) ? shared[lane] : __float2half(0.f);
...
__device__ half block_reduce_sum_f16_f16(half val){
    ...
    float val_f32 = warp_reduce_sum_f16_f16(val);
    if(lane == 0)
        shared[warp] = val_f32;
    __syncthreads();
    val_f32 = (lane < NUM_WARPS) ? shared[lane] : 0.f;
    val_f32 = warp_reduce_sum_f16_f16<NUM_WARPS>(val_f32);
    return val_f32;
}
