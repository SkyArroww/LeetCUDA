...
__device__: 
    - 表示该函数只能在设备(GPU)上执行
    - 只能从其他__device__函数或__global__函数中调用
    - 不能从主机(CPU)代码调用

__forceinline__:
    - 强制编译器内联函数，即使编译器认为不应该内联
    - 可以减少函数调用开销
    - 对于小型、频繁调用的函数特别有用
    - 可能增加代码体积，但通常能提高性能
...
CUDA half精度操作函数：
__hgt(half a, half b): 
    - half precision greater than
    - 如果a > b返回true(1.0)，否则返回false(0.0)
    - 是half类型的比较操作，等价于float的 >
*/
...
// FP16版本：使用half精度特定的数学运算
// __float2half: 将float转换为half类型
// hexp: half精度的指数函数
// __hmul: half精度的乘法运算
// __hsub: half精度的减法运算
