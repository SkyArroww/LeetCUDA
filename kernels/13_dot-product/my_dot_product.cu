...
__global__ void dot_prod_f32_f32_kernel(float *a, float *b, float *y, int N){
    ...
    float prod = (idx < N) ? a[idx] * b[idx] : 0.f;
    ...
    __syncthreads();
    prod = (lane < NUM_WARPS) ? reduce_smem[lane] : 0.f;
    if(warp == 0)
...
__global__ void dot_prod_f32x4_f32_kernel(float *a, float *b, float *y, int N){
    ...
    float4 reg_a = FLOAT4(a[idx]), reg_b = FLOAT4(b[idx]);
    float prod = 0.f;
    if(idx + 3 < N){
        prod = reg_a.x * reg_b.x + 
               reg_a.y * reg_b.y +
               reg_a.z * reg_b.z +
               reg_a.w * reg_b.w;
    }
...
__global__ void dot_prod_f16_f32_kernel(half *a, half *b, float *y, int N){
    ...
    half prod_f16 = (idx < N) ? __hmul(a[idx], b[idx]) : __float2half(0.f);
    int warp = tid / WARP_SIZE;
    int lane = tid % WARP_SIZE;
    float prod = warp_reduce_sum_f16_f32(prod_f16);
