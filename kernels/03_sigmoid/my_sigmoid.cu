...
// FP32 sigmoid数值范围限制:
// 当x > MAX_EXP_F32时, exp(-x)接近0, sigmoid(x)≈1
// 当x < MIN_EXP_F32时, exp(-x)溢出, sigmoid(x)≈0
#define MAX_EXP_F32 88.3762626647949f
#define MIN_EXP_F32 -88.3762626647949f

// FP16 sigmoid数值范围限制:
// 当x > MAX_EXP_F16时, exp(-x)接近0, sigmoid(x)≈1
// 当x < MIN_EXP_F16时, exp(-x)溢出, sigmoid(x)≈0
...
__global__ void sigmoid_f32_kernel(float *x, float *y, int N){
    ...
    if(idx < N){
        float value = x[idx];
        value = fminf(fmaxf(value, MIN_EXP_F32), MAX_EXP_F32);
        y[idx] = 1.0f / (1.0f + expf(-value));
    }
}
...
__global__ void sigmoid_f16_kernel(half *x, half *y, int N){
    ...
    if(idx < N){
        half value = x[idx];
        value = __hmin(__hmax(value, MIN_EXP_F16), MAX_EXP_F16);
        y[idx] = h1 / (h1 + hexp(-value));
    }
}
...
__global__ void sigmoid_f16x2_kernel(half *x, half *y, int N){
    ...
    if(idx + 1 < N){
        ...
        reg_y.x = h1 / (h1 + hexp(-reg_y.x));
        reg_y.y = h1 / (h1 + hexp(-reg_y.y));
...
__global__ void sigmoid_f16x8_kernel(half *x, half *y, int N){
    ...
    if(idx + 7 < N){
        ...
        reg_y_0.x = h1 / (h1 + hexp(-reg_x_0.x));
        reg_y_0.y = h1 / (h1 + hexp(-reg_x_0.y));
        reg_y_1.x = h1 / (h1 + hexp(-reg_x_1.x));
        reg_y_1.y = h1 / (h1 + hexp(-reg_x_1.y));
        reg_y_2.x = h1 / (h1 + hexp(-reg_x_2.x));
        reg_y_2.y = h1 / (h1 + hexp(-reg_x_2.y));
        reg_y_3.x = h1 / (h1 + hexp(-reg_x_3.x));
        reg_y_3.y = h1 / (h1 + hexp(-reg_x_3.y));
...
__global__ void sigmoid_f16x8_pack_kernel(half *x, half *y, int N){
    ...
    if(idx + 7 < N){
        ...
        #pragma unroll
        for(int i = 0; i < 8; ++i){
            half value = __hmin(__hmax(pack_x[i], MIN_EXP_F16), MAX_EXP_F16);
            pack_y[i] = h1 / (h1 + hexp(-value));
        }
