...
__global__ void nms_kernel(const float *boxes, const float *scores, int *keep, int num_boxes, float iou_threshold){
    ...
    const int box_idx = 4 * idx;
    if(idx >= num_boxes)
        return;
    float x1 = boxes[box_idx + 0];
    float y1 = boxes[box_idx + 1];
    float x2 = boxes[box_idx + 2];
    float y2 = boxes[box_idx + 3];
    ...
    for(int i = 0; i < idx; ++i){
        ...
        int now_box_idx = 4 * i;
        float x1_i = boxes[now_box_idx + 0];
        float y1_i = boxes[now_box_idx + 1];
        float x2_i = boxes[now_box_idx + 2];
        float y2_i = boxes[now_box_idx + 3];
